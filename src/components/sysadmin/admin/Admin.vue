  ...
  <v-container grid-list-xs>
    <v-row>
      ...
      <v-col md="8" xs="12">
        <v-btn color="primary" @click="openAdd">
          <v-icon left>add</v-icon>Add Sys Admin
        </v-btn>
      </v-col>
    </v-row>
    <v-row>
    ...
    >
      <v-card>
        ...
        <transition name="slide-fade" mode="out-in">
          <v-card-text :key="dataSingle">{{dataSingle.email}}</v-card-text>
        </transition>
        <v-card-actions>
          <div class="flex-grow-1"></div>
...
import AccountForm from "./adminForm";
...
export default {
  ...
  methods: {
    getDataList() {
      this.tableLoad = true;
      this.axios
        .get(config.baseUri + "/sys-admin/sys-admins", {
    ...
    getDataSingle(id) {
      ...
      this.loader = true;
      this.axios
        .get(config.baseUri + "/sys-admin/sys-admins/" + id, {
          headers: auth.getAuthHeader()
        })
        .then(res => {
