<template>
  <v-container grid-list-xs>
    <v-row>
      <!-- {{authData.data.id}} -->
      <v-col md="8" xs="12">
        <v-btn color="primary" router to="/personnel/worksheet-forms/build">
          <v-icon left>add</v-icon>Create a Worksheet
        </v-btn>
      </v-col>
    </v-row>
    <v-row>
...
export default {
  ...
  methods: {
    ...
    getDataSingle(id) {
      ...
      this.loader = true;
      this.axios
        .get(config.baseUri + "/personnel/as-admin/worksheet-forms/" + id, {
          headers: auth.getAuthHeader()
        })
        .then(res => {
