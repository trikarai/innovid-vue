    ...
    <v-row>
      ...
      <v-col cols="12" md="4" lg="4" xs="12">
        <v-btn
          color="primary"
          router
          :to="'/incubatee/team/' + $route.params.teamId + '/candidate'"
        >
          <v-icon left>people</v-icon>View Invited Candidates
        </v-btn>
      </v-col>
    </v-row>
    <!-- <v-row>
...
export default {
  ...
  watch: {
    teamId() {
      this.$router.replace({
        path: "/incubatee/team/" + this.teamId + "/member"
      });
    },
    $route() {
      this.getDataList();
      ...
      this.axios
        ...
        .then(() => {
          bus.$emit(
            "callNotif",
            "info",
            "Successfully Remove Member from team"
          );
          this.refresh();
        })
        .catch(res => {
