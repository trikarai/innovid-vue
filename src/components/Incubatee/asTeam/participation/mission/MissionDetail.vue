  ...
  <v-container fluid>
    <v-row>
      <v-col class="pl-0" cols="12" xl="12" lg="12" md="12">
        <div
          style="text-align:right;height:0px;position: relative;bottom: 72px;"
        >
          <v-btn
            small
            rounded
          ...
          <template
            v-if="
              $store.getters.getMissionList[dataList.position - 1]
                .submittedJournal !== null
            "
          >
            <v-btn
              class="ml-5"
              ...
              v-if="
                dataList.position !== $store.getters.getMissionLength.toString()
              "
            >
              next mission
              <v-icon right small>mdi-triangle mdi-rotate-90</v-icon>
            </v-btn>
          </template>
          <template v-else>
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <span v-bind="attrs" v-on="on">
                  <v-btn class="ml-5" small rounded :disabled="true">
                    next mission
                    <v-icon right small>mdi-triangle mdi-rotate-90</v-icon>
                  </v-btn>
                </span>
              </template>
              <span>Submit a worksheet journal to continue</span>
            </v-tooltip>
          </template>
        </div>
